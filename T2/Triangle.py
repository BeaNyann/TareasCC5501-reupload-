...
# Encuentra en el triangulo t si posee al vertice v
# Triangle x Vertex -> bool
def find_new_vertex(t, v):
    vertexes = t.vertexes
    for i in range(3):
        if vertexes[i].x == v.x and vertexes[i].y == v.y:
            return True
    return False
...
# Encuentra en el triangulo t si posee al vecino n
# Triangle x Triangle -> bool
def find_new_neighbor(t, n):
    neighbors = t.neighbors
    for i in range(3):
        if find_new_vertex(neighbors[i], n.vertexes[0]) and find_new_vertex(neighbors[i], n.vertexes[1]) and find_new_vertex(neighbors[i], n.vertexes[2]):
            return True
    return False
